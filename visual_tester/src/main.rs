...
fn main() -> opencv::Result<()> {
    ...
    let config = PipelineConfig {
        ...
        chunk_width: 10,
        chunk_height: 10,
        new_age_threshold: 5, // A blob is "New" for its first 5 frames.
        behavioral_anomaly_threshold: 3.0, // Z-score for behavioral change.
...
fn draw_tracked_blobs(frame: &mut Mat, tracked_blobs: &[TrackedBlob], chunk_w: u32, chunk_h: u32) {
    for blob in tracked_blobs {
        ...
        // Assign a color based on the blob's state.
...
/// Generates a color based on the blob's TrackedState.
