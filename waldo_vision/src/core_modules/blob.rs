// THEORY:
// The `Blob` module is the primary component of the Spatial Grouping Layer.
// Its purpose is to transform the purely temporal, single-point-in-space analysis
// from the `SmartChunk` layer into a spatially coherent object. A `Blob`
// represents a single, contiguous region of anomalous activity at a specific
// moment in time.
//
// Key architectural principles:
// 1.  **Spatial Cohesion**: A `Blob` is a collection of `SmartChunk`s that are
...
//     now work with a single `Blob` object that has clearly defined properties
//     like a bounding box, center of mass, and total area.
// 3.  **Stateless Data Container**: Much like `Pixel` and `Chunk`, the `Blob`
...
//     behaviors.
// 4.  **Input for the Next Layer**: A list of `Blob`s is the final output of
